...
local function extract_args(form)
    ...
    for i = 3, #form do
        ...
        if type(arg) == "table" then
            ...
            elseif type_str == "ref.null" then
                args[#args + 1] = { type = "ref", value = nil }
            elseif type_str == "ref.extern" then
                args[#args + 1] = {
                    type = "ref",
                    value = ffi.cast("void*", tonumber(val_str) or 0),
                }
            end
        end
    end
...
local function values_equal(got, expected, typ)
    if typ == "f32" then
        if got == nil or expected == nil then return false end
        ...
        local okg, fg = pcall(ffi.new, "float[1]", got)
        local oke, fe = pcall(ffi.new, "float[1]", expected)
        if not okg or not oke then return false end
        return fg[0] == fe[0]
    elseif typ == "f64" then
        if got == nil or expected == nil then return false end
        if got ~= got and expected ~= expected then return true end
...
local function run_wast(path)
    ...
    local max_segments = tonumber(os.getenv("POT_WAST_MAX_SEGMENTS") or "")
    local trace_segments = os.getenv("POT_WAST_TRACE_SEGMENTS") == "1"
    local file_name = path:match("([^/]+)$") or path
    local exports = nil
    local registered = {}
    local passed, failed, skipped, errors = 0, 0, 0, 0

    local spectest_global_i32 = global(int32)
    local spectest_global_i64 = global(int64)
    local spectest_global_f32 = global(float)
    local spectest_global_f64 = global(double)
    local test_global_i32 = global(int32)
    local test_global_f32 = global(float)
    local test_global_mut_i64 = global(int64)
    spectest_global_i32:set(666)
    spectest_global_i64:set(ffi.cast("int64_t", 666))
    spectest_global_f32:set(666.6)
    spectest_global_f64:set(666.6)
    test_global_i32:set(666)
    test_global_f32:set(666.6)
    test_global_mut_i64:set(ffi.cast("int64_t", 666))
    local terra spectest_print() end
    local terra spectest_print_i32(_ : int32) end
    local terra spectest_print_i64(_ : int64) end
    local terra spectest_print_f32(_ : float) end
    local terra spectest_print_f64(_ : double) end
    local terra spectest_print_i32_f32(_ : int32, _2 : float) end
    local terra spectest_print_f64_f64(_ : double, _2 : double) end
    local terra test_func() end
    local terra test_func_i32(_ : int32) end
    local terra test_func_f32(_ : float) end
    local terra test_func_to_i32() : int32 return 0 end
    local terra test_func_to_f32() : float return 0.0f end
    local terra test_func_i32_to_i32(x : int32) : int32 return x end
    local terra test_func_i64_to_i64(x : int64) : int64 return x end

    local function make_host_functions()
        local host = {
            ["spectest.print"] = spectest_print,
            ["spectest.print_i32"] = spectest_print_i32,
            ["spectest.print_i64"] = spectest_print_i64,
            ["spectest.print_f32"] = spectest_print_f32,
            ["spectest.print_f64"] = spectest_print_f64,
            ["spectest.print_i32_f32"] = spectest_print_i32_f32,
            ["spectest.print_f64_f64"] = spectest_print_f64_f64,
            ["spectest.global_i32"] = spectest_global_i32,
            ["spectest.global_i64"] = spectest_global_i64,
            ["spectest.global_f32"] = spectest_global_f32,
            ["spectest.global_f64"] = spectest_global_f64,
            ["spectest.memory"] = { initial = 1, maximum = 2 },

            -- Common imports from imports.wast.
            ["test.func"] = test_func,
            ["test.func-i32"] = test_func_i32,
            ["test.func-f32"] = test_func_f32,
            ["test.func->i32"] = test_func_to_i32,
            ["test.func->f32"] = test_func_to_f32,
            ["test.func-i32->i32"] = test_func_i32_to_i32,
            ["test.func-i64->i64"] = test_func_i64_to_i64,
            ["test.global-i32"] = test_global_i32,
            ["test.global-f32"] = test_global_f32,
            ["test.global-mut-i64"] = test_global_mut_i64,
        }

        for mod_name, mod_exports in pairs(registered) do
            for fn_name, fn in pairs(mod_exports) do
                host[mod_name .. "." .. fn_name] = fn
            end
        end
    ...
    local function should_skip_invoke(fn_name, args, expected)
        return false
    end

    for seg_idx, seg in ipairs(segments) do
        if max_segments and seg_idx > max_segments then
            break
        end
        -- Determine type
        local trimmed = seg:match("^%s*(.-)%s*$")
        if trace_segments then
            print(string.format("  [seg %d] %.32s", seg_idx, trimmed:gsub("%s+", " ")))
        end
        ...
        if trimmed:sub(1, 7) == "(module" then
            ...
            wf:write(trimmed)
            wf:close()
            local err_path = os.tmpname() .. ".stderr"
            local ret = os.execute("wat2wasm --enable-all " .. string.format("%q", wat_path)
                .. " -o " .. string.format("%q", wasm_path)
                .. " 2> " .. string.format("%q", err_path))
            if ret == 0 or ret == true then
                local wf2 = io.open(wasm_path, "rb")
                local bytes = wf2:read("*a")
                wf2:close()
                local ok, result = pcall(POT.load_module, bytes, make_host_functions())
                if ok then
                    exports = result
                else
                ...
                local emsg = ef and ef:read("*a") or ""
                if ef then ef:close() end
                emsg = (emsg or ""):gsub("%s+$", "")
                if emsg == "" then
                    print("  WAT2WASM FAILED for module")
                else
                    print("  WAT2WASM FAILED: " .. emsg)
                end
                exports = nil
                errors = errors + 1
            end
        ...
        elseif trimmed:sub(1, 9) == "(register" then
            if exports then
                local tokens = tokenize(trimmed)
                local form = parse_sexpr(tokens, 1)
                local reg_name = nil
                if type(form) == "table" and #form >= 2 and type(form[2]) == "string" then
                    reg_name = form[2]:gsub("^\"", ""):gsub("\"$", "")
                end
                if reg_name and reg_name ~= "" then
                    registered[reg_name] = exports
                end
            end

        elseif trimmed:sub(1, 7) == "(invoke" then
            if exports then
                local tokens = tokenize(trimmed)
                local invoke_form = parse_sexpr(tokens, 1)
                if type(invoke_form) == "table" and invoke_form[1] == "invoke" then
                    ...
                    if fn then
                        ...
                        if ok then
                            ...
                            failed = failed + 1
                            io.write(string.format("  FAIL: invoke %s(%s) => ERROR: %s\n",
                                fn_name,
                        ...
                        if not fn then
                            ...
                            for i = 3, #form do
                                ...
                                if type(exp) == "table" then
                                    ...
                                    elseif exp[1] == "ref.null" then
                                        expected[#expected + 1] = { type = "ref", value = nil }
                                    elseif exp[1] == "ref.extern" then
                                        expected[#expected + 1] = {
                                            type = "ref",
                                            value = ffi.cast("void*", tonumber(exp[2]) or 0),
                                        }
                                    end
                                end
                            end
                            ...
                            local skip_this = should_skip_invoke(fn_name, args, expected)
                            if skip_this then
                                skipped = skipped + 1
                            else
                                if not ok then
