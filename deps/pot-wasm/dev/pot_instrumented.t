...
section_parsers[6] = function(mod, bytes, p, pend)
    ...
    for i = 1, count do
        local content_type = wasm_types[bytes:byte(p)]; p = p + 1
        local mutability = bytes:byte(p); p = p + 1
        -- Parse init expression (simplified: const + end)
        local init_val = 0
        local init_op = bytes:byte(p)
        if init_op == 0x41 then -- i32.const
            p = p + 1
            init_val, p = decode_sleb128(bytes, p)
        elseif init_op == 0x42 then -- i64.const
            p = p + 1
            init_val, p = decode_sleb128_i64(bytes, p)
        elseif init_op == 0x43 then -- f32.const
            p = p + 1
            init_val = read_f32(bytes, p); p = p + 4
        elseif init_op == 0x44 then -- f64.const
            p = p + 1
            init_val = read_f64(bytes, p); p = p + 8
        end
...
section_parsers[9] = function(mod, bytes, p, pend)
    local count; count, p = decode_uleb128(bytes, p)
    for i = 1, count do
        local table_idx; table_idx, p = decode_uleb128(bytes, p)
        -- Parse offset init expr (simplified: i32.const + end)
        local offset = 0
        if bytes:byte(p) == 0x41 then
            p = p + 1
            offset, p = decode_sleb128(bytes, p)
        end
        assert(bytes:byte(p) == 0x0B, "expected end of elem init expr")
        p = p + 1
        local num_elems; num_elems, p = decode_uleb128(bytes, p)
        local elems = {}
        for j = 1, num_elems do
            elems[j], p = decode_uleb128(bytes, p)
            elems[j] = elems[j] + 1  -- 1-indexed
        end
        mod.elements[i] = { table_idx = table_idx, offset = offset, elems = elems }
    end
end
...
section_parsers[11] = function(mod, bytes, p, pend)
    local count; count, p = decode_uleb128(bytes, p)
    for i = 1, count do
        local mem_idx; mem_idx, p = decode_uleb128(bytes, p)
        -- Parse offset init expr (simplified: i32.const + end)
        local offset = 0
        if bytes:byte(p) == 0x41 then
            p = p + 1
        ...
        assert(bytes:byte(p) == 0x0B, "expected end of data init expr")
        p = p + 1
        local data_len; data_len, p = decode_uleb128(bytes, p)
        local data = bytes:sub(p, p + data_len - 1)
        p = p + data_len
        mod.datas[i] = { mem_idx = mem_idx, offset = offset, data = data }
    end
end

local function parse_wasm(bytes) _tic('parse_wasm')
    local mod = {
        types    = {},
        imports  = {},
        funcs    = {},
...
local function compile_function(mod, func_idx, module_env) _tic('compile_function')
...
opcode_handlers[0x20] = function(stk, stmts, locals, bc, ip, ...)
    ...
    -- Snapshot the local now: WASM stack values are immutable once pushed.
...
opcode_handlers[0x22] = function(stk, stmts, locals, bc, ip, ...)
    ...
    local val = stk.pop()
    stmts:insert(quote [locals[idx].sym] = [val] end)
    -- Push a snapshot of the assigned value, not a live local reference.
    local tmp = symbol(locals[idx].type, "local_tee")
    stmts:insert(quote var [tmp] = [locals[idx].sym] end)
    stk.push(`[tmp])
    return ip
end
...
opcode_handlers[0x11] = function(stk, stmts, locals, bc, ip, mem,
    ...
    stmts:insert(quote var [fn_ptr_sym] = fn_table[ [idx] ] end)
...
opcode_handlers[0x3F] = function(stk, stmts, locals, bc, ip,
                                  mem, mem_size, ...)
    ip = ip + 1  -- skip reserved byte
    local pages = symbol(int32, "mem_pages")
    stmts:insert(quote
        var [pages] = [int32](mem_size / 65536)
    end)
    stk.push(`[pages])
    return ip
end
...
opcode_handlers[0x40] = function(stk, stmts, locals, bc, ip,
    ...
    stmts:insert(quote
        var [old_pages] = [int32](mem_size / 65536)
        var new_size = mem_size + [int64]([delta]) * 65536
        var new_ptr = [&uint8](C.realloc(mem, [uint64](new_size)))
        if new_ptr == nil then
            [old_pages] = -1
        else
            -- Zero the new pages
            Cstr.memset([&opaque](new_ptr + mem_size), 0, [uint64]([delta]) * 65536)
            mem = new_ptr
            mem_size = new_size
        end
    end)
...
local function init_memory(mod, module_env)
    ...
    elseif #mod.memory == 0 then
        -- No memory declared, check imports
        local has_mem = false
        for _, imp in ipairs(mod.imports) do
            if imp.kind == "memory" then has_mem = true; break end
        end
        if not has_mem then
            -- No memory at all
    ...
    for _, seg in ipairs(mod.datas) do
        ...
        data_inits:insert(quote
            Cstr.memcpy([&opaque](mem_ptr + [offset]), [&opaque](&data_arr), [data_len])
        end)
    end
...
local function ftype_to_c_decl(ftype, name)
    if #ftype.results > 1 then
        error("multi-value functions are not supported in C ABI: " .. name)
    end
    local ret = "void"
    if #ftype.results == 1 then
    ...
    local params = {}
    if #ftype.params == 0 then
        params[1] = "void"
    else
        for i, T in ipairs(ftype.params) do
            params[i] = terra_type_to_c(T) .. " a" .. i
        end
    end
    return ret .. " " .. name .. "(" .. table.concat(params, ", ") .. ");"
end
...
local function init_table(mod, module_env)
    ...
    local table_size = 0
    if mod.tables[1] then
        table_size = mod.tables[1].initial
    end
    for _, elem in ipairs(mod.elements) do
        local needed = elem.offset + #elem.elems
        if needed > table_size then table_size = needed end
    end

    local tbl = global((&opaque)[table_size])
    module_env.fn_table = tbl
...
local function compile_module_core(wasm_bytes, host_functions, opts) _tic('compile_module_core')
    ...
    local mem_init = init_memory(mod, module_env)
    local tbl_init = init_table(mod, module_env)

    local generated_c_imports = {}
    ...
    if opts.auto_c_imports then
        generated_c_imports = add_missing_c_imports(mod, host_functions, opts)
    end
    ...
    for i, func in ipairs(mod.funcs) do
        ...
        if noopt and entry.fn.setoptimized then
            entry.fn:setoptimized(false)
        end
    ...
    local init_globals = terra()
        escape
            for i, g in ipairs(mod.globals) do
                local gsym = module_env.globals[i].sym
                emit quote [gsym] = [g.type]([g.init]) end
            end
        end
    end
        ...
        escape
            for _, elem in ipairs(mod.elements) do
                for j, fn_idx in ipairs(elem.elems) do
                    ...
                    if fn_entry and module_env.fn_table then
                        local tbl = module_env.fn_table
                        local offset = elem.offset + j - 1
                        emit quote
                            tbl[offset] = [&opaque]([fn_entry.fn])
                        end
                    end
                end
            end
    ...
    return {
        ...
        generated_c_imports = generated_c_imports,
    }
end
...
function POT.compile_module(wasm_bytes, host_functions, opts)
    ...
    return {
        ...
        generated_c_imports = compiled.generated_c_imports,
    }
end

function POT.load_module(wasm_bytes, host_functions)
    local compiled = compile_module_core(wasm_bytes, host_functions, {
        auto_wasi = true,
    })
    -- Run initialization
    compiled.mem_init()
    ...
    -- Force compilation
    for name, fn in pairs(compiled.exports) do
        fn:compile()
    end
...
function POT.save_c_module(wasm_bytes, out_obj, opts)
    opts = opts or {}
    assert(type(out_obj) == "string" and #out_obj > 0, "out_obj path required")

    local c_prefix = opts.c_prefix or "po_"
    local export_prefix = opts.export_prefix or (c_prefix .. "export_")
    local init_symbol = opts.init_symbol or (c_prefix .. "init")
    local deinit_symbol = opts.deinit_symbol or (c_prefix .. "deinit")
    local mem_symbol = opts.memory_symbol or (c_prefix .. "memory")
    local mem_size_symbol = opts.memory_size_symbol or (c_prefix .. "memory_size")

    local compile_opts = {
        auto_wasi = opts.auto_wasi,
        auto_c_imports = true,
        import_prefix = opts.import_prefix or (c_prefix .. "import_"),
    }
    local module = POT.compile_module(wasm_bytes, opts.host_functions or {}, compile_opts)
    local mod = module.mod

    local symbol_map = {
        [init_symbol] = module.init_fn,
        [deinit_symbol] = module.deinit_fn,
        [mem_symbol] = module.memory_fn,
        [mem_size_symbol] = module.memory_size_fn,
    }
    ...
    local used = {
        ...
        [deinit_symbol] = true,
        [mem_symbol] = true,
        [mem_size_symbol] = true,
    }

    local function unique_symbol(base)
        local sym = base
        local n = 2
        while used[sym] do
            sym = base .. "_" .. n
    ...
    local exported = {}
    for _, wasm_name in ipairs(export_names) do
        local exp = mod.exports[wasm_name]
        local entry = module.exports[wasm_name]
        local ftype = resolve_function_type(mod, exp.index)
        assert(ftype, "failed to resolve type for export: " .. wasm_name)
        if #ftype.results > 1 then
            error("multi-value export not supported in C ABI: " .. wasm_name)
        end
        local c_sym = unique_symbol(export_prefix .. sanitize_c_ident(wasm_name))
        symbol_map[c_sym] = entry
        exported[#exported + 1] = {
            wasm_name = wasm_name,
            c_symbol = c_sym,
            ftype = ftype,
    ...
    terralib.saveobj(out_obj, "object", symbol_map, opts.link_args or {})

    local header_path = opts.header_path
    if header_path then
        local module_name = opts.module_name
        if not module_name or module_name == "" then
            module_name = "po_module"
        end
        local guard = sanitize_c_ident(module_name):upper() .. "_H"
        local lines = {}
        ...
        lines[#lines + 1] = "#endif"
        lines[#lines + 1] = ""
        lines[#lines + 1] = "void " .. init_symbol .. "(void);"
        lines[#lines + 1] = "void " .. deinit_symbol .. "(void);"
        lines[#lines + 1] = "uint8_t* " .. mem_symbol .. "(void);"
        lines[#lines + 1] = "int64_t " .. mem_size_symbol .. "(void);"
        lines[#lines + 1] = ""

        if #module.generated_c_imports > 0 then
            lines[#lines + 1] = "/* Host-provided imports */"
            for _, imp in ipairs(module.generated_c_imports) do
                lines[#lines + 1] = ftype_to_c_decl(imp.ftype, imp.c_symbol)
            end
            lines[#lines + 1] = ""
        end

        lines[#lines + 1] = "/* WASM exports */"
        for _, e in ipairs(exported) do
            lines[#lines + 1] = ftype_to_c_decl(e.ftype, e.c_symbol)
        end
        lines[#lines + 1] = ""
        lines[#lines + 1] = "#ifdef __cplusplus"
        lines[#lines + 1] = "}"
        ...
        local hf = assert(io.open(header_path, "wb"))
        hf:write(table.concat(lines, "\n"))
        hf:close()
    end
    ...
    return {
        ...
        exports = exported,
        imports = module.generated_c_imports,
        init_symbol = init_symbol,
        deinit_symbol = deinit_symbol,
        memory_symbol = mem_symbol,
        memory_size_symbol = mem_size_symbol,
    }
end

function POT.save_c_module_file(wasm_path, out_obj, opts)
    local f = io.open(wasm_path, "rb")
    assert(f, "cannot open: " .. wasm_path)
    local bytes = f:read("*a")
    f:close()
    POT._timings = _get_timings

return POT.save_c_module(bytes, out_obj, opts)
end

function POT.load_file(path, host_functions)
    local f = io.open(path, "rb")
    assert(f, "cannot open: " .. path)
    local bytes = f:read("*a")
    f:close()
    return POT.load_module(bytes, host_functions)
end
...
POT._toc('parse_wasm') parse_wasm = parse_wasm
